...
| lerobot_composition | paused after qualified no-video seed | 2026-09-10 | A locked one-frame v3 seed reaches metadata, episode/data loading, PyTorch tensor materialization, task lookup and a one-worker DataLoader; four controls fail at exact earlier stages. Video remains source-only. | No compute. A separately approved R8 brief must lock the native decoder environment and prove one video frame plus missing/out-of-tolerance controls. |
