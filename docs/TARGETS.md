...
| lerobot_composition | maintainer-paused after qualified video seed | 2026-09-11 | A locked one-frame v3 video seed reaches metadata, a Parquet row, a resolved H.264 MP4, PyAV float32 CHW decoding, task lookup and a one-worker DataLoader; missing, truncated and late-timestamp controls fail at exact earlier stages. | No compute. Reopen only for a meaningful upstream boundary change or a separately reviewed brief with named states, dedup rules and a new cap; known video/timestamp cases remain controls. |
