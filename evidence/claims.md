...
- observation: 23-byte input {"decoder":<malformed>} triggers PanicException at decoders/mod.rs:90. DecoderHelper::deserialize(...).expect("Helper").
- boundary: DoS only (panic on load); safe Rust, no memory-safety or code-execution impact. Establishes one site, not the full class (see C-0002, C-0004).
- refs: PRIVATE_findings/0002-0003-DISCLOSURE-REPORT.md

### C-0002  (verified | severity: low | status: reported)
**A Precompiled normalizer with missing OR malformed precompiled_charsmap panics at normalizers/mod.rs:143 via .expect() (finding 0003).**

- target: tokenizers / Tokenizer::from_str
- date: 2026-08-13
- provenance:
...
- observation: {"type":"Precompiled"} normalizer panics: Precompiled Error("missing field precompiled_charsmap"). Correct pattern commented out one line above (142); adjacent Replace arm (146) uses map_err(serde::de::Error::custom)?.
- boundary: DoS only. Same class as C-0001. Reproduced across three released versions (strengthens generality). Does not establish reachability in any specific downstream application.
- refs: PRIVATE_findings/0002-0003-DISCLOSURE-REPORT.md

### C-0003  (verified | severity: low | status: reported)
**The .expect-on-untrusted-deserialize panic class is bounded to exactly two tokenizer components: normalizers and decoders.**
...
- provenance:
  ...
  - claim:C-0001
  - claim:C-0002
- observation: All 20 pre_tokenizer/post_processor variants and models reject malformed input cleanly (normal Exception, no PanicException) across 20+ fielded probes. normalizers/mod.rs:101 and :104 verified NOT reachable (reject cleanly).
...
- provenance:
  ...
  - claim:C-0011
- observation: Original 0005 writeup listed operator/ (503) and operator% (508). The deeper campaign FPE'd at 1375 inside BinaryOpExpr::do_evaluate, a second independent modulo implementation. Non-sanitized test confirmed real SIGFPE.
- boundary: Corrects scope of C-0011; does not add a new finding (same class, same crash). Matters for fix completeness (a partial fix of only the Value operators would leave the switch sites crashing).
...
- observation: Type-mismatched state -> AttributeError "'X' object has no attribute 'keys'". All flagged cases (float/None/bytes/str/bool/list/int) are one root cause. 200k further inputs found no other fault.
- boundary: NOT counted as a finding: program does not crash, a caller's except still catches it, wrong exception type only. Worth at most a one-line upstream PR.
- refs: targets/flax_checkpoint/README.md

### C-0021  (verified | severity: none | status: active)
**minja's integer arithmetic (operator*, and inf->long via json cast) invokes UB on extreme operands but does NOT crash a release build; not a finding.**
...
- provenance:
  ...
  - source-read:reachability confirmed by source-read, a crafted config.json with num_attention_heads 0 reaches the division
...
- date: 2026-08-28
- provenance:
  - execution-run:environment not recorded; executed sampling_params _validate_allowed_token_ids with tokenizer None passed an out-of-vocab id 999999999 with no rejection, in vllm 0.28.0
  - execution-run:environment not recorded; executed the same validator with a tokenizer present and it raised VLLMValidationError rejecting the out-of-vocab id, confirming the defended path
  - execution-run:environment not recorded; reproduced the gpu_input_batch operation mask req_index indexed by the id equals False on a bool tensor of width 151936, which raised IndexError index 999999999 out of bounds
...
**The weights_only unpickler is robust across blind, structure-aware, and coverage-stable fuzzing; no defect found.**

- target: pytorch / torch._weights_only_unpickler.load
- date: 2026-08-13
- provenance:
...
- observation: Three independent harnesses, 6M+13M+ runs, all clean. Restricted unpickler needs LEGACY GLOBAL opcode not STACK_GLOBAL; persistent_load must return a TYPED storage. Coverage ceiling cov:57 (shallow corridor).
- boundary: Robust on the reachable weights_only surface at stated effort. Does NOT cover the full torch.load (non-weights_only) surface, nor container zip-surgery beyond what was tested (CVE-2026-24747 defended on the tested machine).
- refs: targets/pytorch/

### C-0031  (verified | status: active)
**PretrainedConfig.from_dict is robust over a 487k-run campaign; no defect found (with code-exec keys deliberately scoped out).**

- target: transformers_config / PretrainedConfig.from_dict
- date: 2026-08-13
- provenance:
  - machine-run:Atheris campaign 487k runs clean
  - source-read:configuration_utils.py:861 entry point verified
- observation: Clean over 487k runs. Scope guard strips code-exec keys (auto_map, trust_remote_code, etc.), deliberately avoiding the config-RCE surface (CVE-2026-4372/5241/1839).
- boundary: Robust on the non-code-exec config-parse surface. Deliberately does NOT test the config-RCE surface (out of scope by design). Not a statement about the RCE keys.
- refs: targets/transformers_config/

### C-0032  (verified | status: active)
**flax msgpack_restore array-reconstruction is robust; malformed shape/dtype/buffer reject cleanly.**

- target: flax_checkpoint / msgpack_restore
- date: 2026-08-13
- provenance:
...
- observation: 23.4M runs no crash. Malformed shape/buffer mismatch, huge shape, object dtype all reject cleanly (numpy frombuffer defenses hold). Coverage flat (cov:33), numpy-C blind spot.
- boundary: Robust but a SHALLOW negative: coverage flat because array work is in numpy C code with weak coverage feedback. Does not deeply exercise flax's own logic (the from_state_dict path C-0020 covers that).
- refs: targets/flax_checkpoint/

### C-0033  (verified | status: active)
**BERT fast (Rust) vs slow (Python) tokenizers agree across historically divergent Unicode categories; no supply-chain divergence found.**
...
- observation: fast==slow held across all probes. Byte-level BPE (GPT2) pairing INCONCLUSIVE offline (could not build a valid divergent baseline without network fetch).
- boundary: Fast/slow well-reconciled at reachable BERT surfaces. GPT2 byte-level pairing is inconclusive, NOT confirmed robust. Does not cover all tokenizer families.
- refs: targets/tokenizers_differential/

### C-0034  (verified | status: active)
**Current llama.cpp GGUF is hardened against resource-exhaustion (declared-huge) inputs; bounds and overflow guards are present.**

- target: gguf / gguf_init_from_buffer
- date: 2026-08-13
- provenance:
...
- observation: All mainstream format libs in OSS-Fuzz, including the juicy surfaces (numpy .npy/dtype, keras deserialize RCE, HDF5 file layer). GGUF/tokenizers-Rust/ flax/minja/orbax/jax/msgpack/dreamerv3/lerobot all ABSENT (404).
...
### C-0043  (verified | status: reported)
...
- provenance:
  ...
  - source-read:web huggingface diffusers security advisory GHSA-j7w6-vpvq-j3gm None.py trust_remote_code bypass verified against a DDUF pipeline
  - source-read:web CVE-2026-44513 and CVE-2026-44827 and CVE-2026-45804 FaceHugger by Zafran Security, config-as-code TOCTOU, fixed in diffusers 0.38.0 May 2026
  - source-read:web parallel transformers config injection CVE-2026-4372 via attn implementation internal, same root cause
...
### C-0061  (verified | status: active)
**At the tested Isaac-GR00T revision, the policy server's deserialization boundary resists three specific classes, measured on the versions recorded below. MsgSerializer rejects object-dtype NPY and msgpack-numpy payloads before either decode path can unpickle attacker data. msgpack rejects the tested array32 and map32 declaration bombs. np.load and frombuffer reject a declared-shape-versus-actual-bytes mismatch with zero measured allocation. This is scoped to those tested inputs and versions, not a general safety claim, and it says nothing about authentication, which is tracked separately.**

- target: nvidia_groot / gr00t/policy/server_client.py MsgSerializer.from_bytes and the ZMQ run loop
- date: 2026-08-28
- provenance:
  - environment:versions tested GR00T commit 51d4c89f72fda44cbf77285c6a8114b52676b8a1, python 3.10.12, numpy 2.2.6, msgpack 1.1.0, msgpack-numpy 0.4.8
  - source-read:server_client.py MsgSerializer._safe_decode rejects object-dtype ndarray payloads two ways before np.load or mnp.decode can unpickle, np.load is called with allow_pickle False
  - machine-run:msgpack unpackb rejects a 5-byte array32 header declaring 4.29e9 elements with ValueError exceeds max_array_len, same for map32, on msgpack 1.1.0
  - machine-run:measured peak RSS delta of np.load allow_pickle False on crafted NPY headers declaring 1e6, 1e8, and 5e8 float64 elements with no data was 0.0 MB each, all rejected with ValueError before allocation, on numpy 2.2.6
  - machine-run:msgpack-numpy mnp.decode uses frombuffer which raises TypeError buffer is too small when declared shape exceeds actual data bytes, on msgpack-numpy 0.4.8
- observation: Tested in-process against the exact from_bytes entry point with no model and no GPU. The NPY allocation-safety was measured, not inferred, peak RSS did not move for declared shapes up to 5e8 float64 because the data-length check fires before allocation on numpy 2.2.6. This is version-specific and was not re-tested on GR00T's own pinned numpy. The unauthenticated-parse structural point (token is inside the payload, so auth runs after from_bytes) is real but is not a defense and is tracked as a separate finding, not resolved here.
- boundary: Scoped strictly to the three tested classes on the recorded versions. NOT a general claim that the boundary is safe, only that these specific inputs were rejected with zero measured allocation. Not tested, other numpy versions, live-server timing, ZMQ state-machine behavior, and handler internals. Does not cover authentication or authorization, which is a separate finding. A negative result on tested amplification inputs, not proof that no amplification exists.
- refs: docs/FRONTIER.md, docs/THREAT-MODEL.md

## Methods (lessons)

### C-0050  (verified | status: active)
**Structure-STABLE encoding (fixed-layout byte decode so one flipped byte changes one field) turns libFuzzer byte mutations into structural mutations, but only pays off on a rich surface.**
...
- observation: tokenizer.json (branchy) yielded 0003 quickly under the stable generator; the pytorch meta-tensor path (cov ceiling 57) showed no gain.
- boundary: Effective only where the target surface is rich/branchy. On shallow corridors or native-C surfaces (weak coverage feedback) it does not help.
- refs: docs/LESSONS.md

### C-0051  (verified | status: active)
**Dedup crashes by fault LOCATION, not artifact count; many artifacts routinely collapse to one root cause.**
...
- observation: 14,724 GGUF artifacts deduped to one div-by-zero; 147 flax from_state_dict flags to one .keys() site.
- boundary: Location-dedup is necessary for honest counting; it does not by itself judge severity (a location can be benign UB, see C-0021).
- refs: chassis/triage.py, docs/LESSONS.md

### C-0052  (verified | status: active)
**A UBSan flag is not a finding until confirmed to crash a non-sanitized release build; several UBSan hits are defined-in-practice and do not crash.**
...
- observation: Same campaign: signed-overflow and float-cast UBSan hits render values and exit 0 (non-findings); div/mod-by-zero UBSan hits crash with SIGFPE (findings). The non-sanitized test is the discriminator.
- boundary: Distinguishes real crashes from benign UB. Does not address bugs that are memory-unsafe only under specific allocators/platforms; ASan is the check for those.
- refs: docs/LESSONS.md

### C-0053  (observed | status: active)
**A stored crashing input re-crashes on libFuzzer startup replay, so a persistent corpus must be minimized (-merge=1) or restarted from seeds after findings.**

- target: project
- date: 2026-08-13
- provenance:
  - machine-run:accumulated minja corpus segfaulted on startup; empty/fresh corpus ran clean
- observation: Campaigns from an accumulated corpus segfaulted at startup while empty-corpus fuzzing ran 60k clean. Individual seed files ran clean, so a specific accumulated input (or the merge pass) was the trigger.
- boundary: Observed on WSL for the minja target; the exact poisoning input was not isolated. Remedy (fresh/minimized corpus) is confirmed to work; root cause of the dir-replay segfault is not fully established.
- refs: targets/minja_template/README.md

### C-0054  (verified | status: active)
**To fuzz PAST a known finding without disclosing a reproducer, apply the recommended fix to the vendored dependency (a fuzz-blocker) and keep it embargoed with the finding.**
