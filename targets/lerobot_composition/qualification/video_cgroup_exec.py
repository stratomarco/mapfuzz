#!/usr/bin/env python3
"""Fail closed unless the R8 hard resource scope is effective."""
...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit("usage: video_cgroup_exec.py COMMAND [ARG ...]")
    entry = Path("/proc/self/cgroup").read_text(encoding="utf-8").strip()
    if "::" not in entry:
    ...
    if memory_max != str(EXPECTED_MEMORY_MAX) or swap_max != "0" or len(cpu_affinity) != 1:
